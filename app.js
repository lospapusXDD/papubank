...
window.uploadCustomAvatar = function(event) {
    ...
    reader.onload = function(e) {
        ...
        img.onload = async function() {
            // Resize using canvas to keep Firestore document size under 1MB
            const canvas = document.createElement('canvas');
            const ctx = canvas.getContext('2d');
            const maxSize = 256; // 256x256 is perfect for profile pics
            let w = img.width;
            let h = img.height;
            ...
            try {
                const db = window._db;
                await window._fbUpdateDoc(window._fbDoc(db, 'users', currentUser.nick), { avatar: base64 });
                currentUser.avatar = base64;
                
                // Sync UI
                const pAv = document.getElementById('profile-avatar');
                if (pAv) pAv.src = base64;
                const sAv = document.getElementById('sidebar-user-avatar');
                if (sAv) sAv.src = base64;

                loadProfile();
                showToast('Foto personalizada subida ✓', '#00ffaa');
            } catch(err) {
                showToast('Error al subir foto: ' + err.message, '#ff4466');
            }
        };
