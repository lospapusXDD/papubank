...
async function loadChat() {
    ...
    try {
        ...
        msgs.forEach(m => {
            ...
            el.innerHTML = '<div style="font-size:10px;font-weight:700;color:var(--primary);">' + (isMe ? 'Tú' : (m.nick || 'Sistema')) + ' <span style="font-weight:400;color:var(--text-muted);font-size:9px;">' + dateStr + '</span></div><div style="font-size:12px;color:var(--text-main);margin-top:4px;">' + String(m.msg || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</div>';
